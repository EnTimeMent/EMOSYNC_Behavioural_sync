...
function [A,Q,exy] = plotgauss2d(mu, Sigma, varargin)

if all(size(Sigma) ~= [2 2]), error('Sigma must be a 2 by 2 matrix'); end
if length(mu) ~= 2, error('mu must be a 2 by 1 vector'); end

[p, n, plot_opts] = process_options(varargin,'conf',0.9,'num-pts',100);
%holding = ishold;
if (Sigma == zeros(2, 2))
    z = mu; 
else
...
plot(z(1, :), z(2, :), plot_opts{:}) %plot ellipses --> Put in comment for a large data computing
exy=z;
