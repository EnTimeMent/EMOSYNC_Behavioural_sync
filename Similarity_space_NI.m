...
for p=33%34
    for j=1:9
        ...
        load(['/Work/Programming/MatLab/Pauline/MG/D' num2str(p) '/Trial0' num2str(j)])
        %load(['C:\Work\Programming\MatLab\Pauline\MG\D' num2str(p) '\Trial0' num2str(j)]) 
...
%%
for i=1:kk-1
    min_mrk2(i)=min(data_for_histograms(i).vec);
...
for id1=1:numb_of_all_hist
    for id2=id1:numb_of_all_hist
        ...
        h1=hist(data_for_histograms(id1).vec,bins);
...
for i=1:36:kk-1
    ...
    idx3=(i+18):(i+26)
    idx4=(i+27):(i+35)
emd_P1_P2(ctr)= mean(mean(emds(idx1,idx2)));
emd_NI_P1_P2(ctr)= mean(mean(emds(idx3,idx4)));
ctr=ctr+1,
end

%% Compute distance for P1 or P2 
ctr=1;
for i=1:36:kk-1
    idx1=i:(i+8);
    idx2=(i+9):(i+17)
    idx3=(i+18):(i+26)
    idx4=(i+27):(i+35)
emd_P1_Solo_Duo(ctr)= mean(mean(emds(idx1,idx3)));
emd_P2_Solo_Duo(ctr)= mean(mean(emds(idx2,idx4)));
ctr=ctr+1,
end
