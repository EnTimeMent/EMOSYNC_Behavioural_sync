% CreateLearnScenario_Hand
% In order to visualize time series of Hand velocity
...
% Init
SCENARIO.Q = 'Visualize_Hand_VEL_ACC_timeseries';
...
% SCENARIO.features = [SCENARIO.features,[Parameters.FeatureName,'_',namemarker,'_X']...
%     [Parameters.FeatureName,'_',namemarker,'_Y'],[Parameters.FeatureName,'_',namemarker,'_Z']];
% SCENARIO.xpdf = [SCENARIO.xpdf,x];
% SCENARIO.span = [SCENARIO.span,s];
% SCENARIO.X = [SCENARIO.X,X]; 
% SCENARIO.namesfeatures = [SCENARIO.namesfeatures,nf];
...
NORMVELOC = zeros(N,T);
for n = 1 : N
    row = F.VALUES{n,imarker}';
    lrow = length(row);
    disp([n,size(row)])
    NORMVELOC(n,1:lrow) = row;
end
% Update SCENARIO
% TODO: Put here namemarker_X, namemarker_Y, namemarker_Z
SCENARIO.features = [SCENARIO.features,[Parameters.FeatureName,'_',namemarker,'_X']...
...
save(nameS,'SCENARIO','NORMVELOC','NORMACCEL')
